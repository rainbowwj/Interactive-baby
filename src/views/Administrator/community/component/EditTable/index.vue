          ...
          <el-table-column v-for="(v,i) in master_user.columns" :prop="v.field" :label="v.title" :width="v.width" :key="i">
            <template slot-scope="scope">
              <span v-if="scope.row.isSet">
                  <!-- <el-input size="mini" placeholder="请输入内容" v-model="master_user.sel[0][v.field]" > -->
...
export default {
  ...
  watch: {
    getEduApplyByApplyIdList: function (val, oldVal) {
      const changeArray = val.classInfoList.map(item=>{
        return  {
          id: item.sysNo,
          type: item.name,
          addport: item.kiddieCount,
          user: item.graduateDate && item.graduateDate.slice(0,10),
          pwd: item.invitationCode, 
          info: "",
          isSet: false,
          sysNo:item.sysNo,
          _temporary: false
    ...
    this.master_user.sel = JSON.parse(JSON.stringify(changeArray));
    this.tableArr = changeArray;
    },
  },
  async created(){
    ...
    const some = this.getEduApplyByApplyIdList.classInfoList.map(item=>{
      return  {
        id: item.sysNo,
        type: item.name,
        addport: item.kiddieCount,
        user: item.graduateDate.slice(0,10),
        pwd: item.invitationCode, 
        info: "",
        isSet: false,
        sysNo:item.sysNo,
    ...
    this.master_user.sel = JSON.parse(JSON.stringify(some));
    this.tableArr = some;
  },
  computed:{
    ...mapState({
    ...
    pwdChange(row, index, cg) {
        ...
        if (row.isSet) { 
            ( async function () {
                ...
                let eduId = that.$route.params.id
